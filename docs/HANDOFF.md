...
## 2. 현재 상태 (2026-09-19, 본문 찾기 제거 시점)

| 항목 | 값 |
|---|---|
| 브랜치 | `main` = `origin/main`, 작업본 깨끗함. 마지막 코드 변경은 **본문 찾기 제거**(6절 맨 위 기록) |
| 버전 · 태그 | 0.2.0 · `v0.2.0` → `4452e99`. 그 뒤의 변경은 CHANGELOG 의 "미출시" 에 있다(버전은 올리지 않았다) |
| 배포 | https://kihoedu.github.io/kiho-pdf/ — `9c6cae1` 빌드. **지금 main 과 다르다: 배포본에는 본문 찾기가 아직 있다.** 다시 배포해야 반영된다 |
| CI | 본문 찾기 제거 커밋의 결과는 푸시 후 확인할 것. 그 전 마지막 확인은 `c68fdab` 통과(Linux·Chrome) |
| 로컬 검증 | Windows·Edge 에서 타입 검사 · 단위 43 · `npm run e2e` 20개 통과(1.3분) |
| 진행 중인 작업 | 없음 |
...
| D4 | **썸네일 표시 속도**: 2026-09-18 에 소유자가 "검토하라"고 해서 방안을 제시했고 승인 단계에서 멈춰 있다 | 미구현. 측정값과 방안은 3-2 의 2번 |
...
2. **썸네일 표시 속도 개선(소유자 승인 대기 — 3-1 의 D4).** `npm run bench:thumb` 으로 방식을 비교해 두었다.
   측정(300dpi JPEG 스캔본 40쪽, i5-9500 6코어, 헤드리스 Edge=GPU 없음): 지금 방식(PDF.js 워커 1개) **48ms/쪽**,
   PDF.js 워커를 2~4개로 늘리면 37~39ms/쪽(+25%)이지만 메인 스레드 최장 막힘이 23ms → 137ms 로 나빠지고 메모리도 배로 든다(**비추천**),
   내장 JPEG 을 `createImageBitmap(blob, { resizeWidth })` 로 직접 축소 디코딩하면 **18~21ms/쪽(2.4~2.7배)**.
   - 제안한 순서: ① 스캔 쪽 빠른 경로("쪽 전체를 덮는 단일 JPEG"인 쪽만 직접 디코딩, 아니면 지금 방식으로) ② 스크롤 방향 선행 렌더 ③ IndexedDB 영구 캐시.
   - ①은 방향·잘림이 틀릴 위험이 있으므로 두 방식의 결과를 픽셀로 비교하는 테스트를 함께 넣는다.
   - 흑백 1비트(CCITT·JBIG2) 스캔은 재지 않았다. 그런 파일에는 ①의 이득이 없다. 실제 파일은 `node bench/thumb-bench.mjs <파일.pdf>` 로 잰다.
...
| 커밋 | 내용 |
|---|---|
| (이 커밋) | 본문 찾기 제거 + 0.1.0 을 만든 기기의 기록으로 문서의 사실관계를 바로잡음 |
